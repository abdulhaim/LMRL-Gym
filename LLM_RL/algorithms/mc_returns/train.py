...
def dump_state(
    ...
    **loop_state: Dict[Hashable, Any], 
    ...
    # save q_head
    if enable_save:
        create_path(os.path.join(save_dir, 'q_head'))
    # dump q_head config
    with open(get_enabled_save_path(os.path.join(save_dir, 'q_head', 'config.json'), enabled=enable_save), 'w') as f:
        f.write(q_head_model.config.to_json_string())
    # dump train_state
    if save_train_state:
        save_pytree(
            tree=q_head_train_state, 
            path=get_enabled_save_path(os.path.join(save_dir, 'q_head', 'train_state.msgpack'), enabled=enable_save), 
            dtype=save_dtype, 
            sharding=get_sharding_from_model(q_head_model, q_head_train_state), 
        )
    else:
        save_pytree(
            tree=q_head_train_state.params, 
            path=get_enabled_save_path(os.path.join(save_dir, 'q_head', 'params.msgpack'), enabled=enable_save), 
            dtype=save_dtype, 
            sharding=get_sharding_from_model(q_head_model, q_head_train_state.params), 
        )
