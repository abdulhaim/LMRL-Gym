...
import json
import numpy as np
from llm_rl_scripts.chess.env.env import preprocess_state_og, preprocess_move
...
def main(
    ...
    train_data = Seq2SeqIterableDataset.from_str_iterable(
        MapIterable(
            lambda x: (tokenizer.bos_token+preprocess_state_og(x["from_state"]).removeprefix(tokenizer.bos_token), preprocess_move(x['out_text'])), 
