...
def main(
    ...
    use_wandb: bool=True, 
    wandb_project: Optional[str]="fully_observed_online_filtered_bc", 
    ...
    def filtered_bc_dataset_loader(ppo_inference: GPT2InferenceMask, policy: GPT2PPOPolicy) -> MaskIterableDataset:
        ...
        for raw_result in raw_results:
            if raw_result[-1].reward == 0: # success has been achieved
                print('='*25)
        ...
        # top_data_idxs = list(range(len(mask_str_segments)))
        
        # top_data_idxs = [idx for idx in data_idxs if trajectory_rewards[idx] > -50]
        # top_data_idxs = sorted(data_idxs, key=lambda idx: trajectory_rewards[idx], reverse=True)[:int(len(data_idxs))]
