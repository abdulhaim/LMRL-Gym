...
from llm_rl_scripts.text_nav.dataset.utils import data_stream
...
def main(
    ...
    train_data = MaskIterableDataset.blocked_from_str_segments_iterable(
        MapIterable(lambda x: (tokenizer.bos_token + x['in_text'], x['out_text']),
                    FileOpenIterable(convert_path(train_data_path), 'r', pipe=data_stream)), 
        tokenizer, 
    ...
    eval_data = MaskIterableDataset.blocked_from_str_segments_iterable(
        MapIterable(lambda x: (tokenizer.bos_token + x['in_text'], x['out_text']),
                    FileOpenIterable(convert_path(eval_data_path), 'r', pipe=data_stream)), 
        tokenizer, 
        blocking_strategy=BlockingStrategy(
            padding=Padding.RIGHT, 
            truncation=Truncation.LEFT, 
    ...
    env = TextNavEnv(display_location=not is_partial_info,
                     display_inventory=not is_partial_info)
